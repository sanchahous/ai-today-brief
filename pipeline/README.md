...
| `MAX_EMBED_DISTANCE` | 0.12 | 0.05–1 | Cosine distance ceiling for semantic dedup (lower = stricter). 0.20 dropped ~89% genuinely-new items in this narrow niche; 0.12 keeps throughput while auto-removing only near-identical re-words |
...
2. ~~**Cross-day dedup is title-only.**~~ **Fixed** — step 3.5 embeds every pool candidate with `gemini-embedding-001` (768 dims) and drops any candidate whose cosine distance to a previously-published `brief_item_embeddings` row is ≤ `MAX_EMBED_DISTANCE` (default 0.12). The same event re-worded across days now lands close in vector space and is removed before the LLM call. *Residual:* the store is empty until at least one brief has been published; dedup strengthens over the first few days of use. *Calibration note:* the ceiling was lowered 0.20 → 0.12 after a week of prod logs showed 0.20 auto-dropping ~89% genuinely-new stories (the archive saturates vector space in this narrow niche). Migration 039 also bounds `match_relevant_item` to a **30-day recency window** (was: all published items, any day) so the comparison set — and thus the false-positive rate — stays bounded as the archive grows; today's pack is still matched unbounded.
