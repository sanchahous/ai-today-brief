  ...
  properties: {
    results: {
      ...
      items: {
        ...
        properties: {
          ref: { type: SchemaType.NUMBER },
          unsupported_claims: { type: SchemaType.ARRAY, items: STR },
        },
...
List ONLY specific factual claims (numbers, prices, limits, dates, named features) that the
source does NOT support (≤ 120 chars each). General definitional statements and
widely-established background knowledge are fine — do not flag them.

An empty unsupported_claims array is the expected result for a good entry.
...
/** Parse verify output into ref → claims, dropping hallucinated refs. */
export function parseConceptVerify(text: string, validRefs: Set<number>): Map<number, string[]> {
  const parsed: unknown = JSON.parse(text);
