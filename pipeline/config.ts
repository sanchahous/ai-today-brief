...
export function loadPipelineConfig(
  ...
  return {
    ...
    // 0.15 (was 0.12, was 0.20). 0.20 gutted throughput — in this narrow AI/dev
    // niche almost every fresh item sits within 0.20 cosine of *something*
    // already published, so it discarded ~89% genuinely NEW stories and the
    // brief shipped ~1 item/edition; 0.12 fixed that but proved too loose in
    // practice — real duplicates (the same event re-told days apart) were
    // slipping through in the 0.12–0.20 band and reaching the archive. 0.15
    // is a cautious step back up: it should catch more of those without
    // reintroducing 0.20's throughput collapse. The exact-URL dedup
    // (dropKnownUrls) and the human ✅/❌ review gate catch the residual cases.
    maxEmbedDistance: floatIn(env.MAX_EMBED_DISTANCE, 0.15, 0.05, 1),
