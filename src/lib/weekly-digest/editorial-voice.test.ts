...
describe('voicePromptBlock', () => {
  it('assembles the EN block without injecting the copy-prone full exemplar', () => {
    const block = voicePromptBlock('en');
    expect(block).toContain('VOICE');
    expect(block).toContain('sharp, well-read colleague');
    expect(block).not.toContain('STYLE REFERENCE ONLY');
    expect(block).toContain('Never invent a scene');
  ...
  it('assembles the UK block natively, not as a translation of the EN block', () => {
    ...
    expect(block).not.toContain('ЛИШЕ СТИЛЬОВИЙ ЗРАЗОК');
    expect(block).toContain('Не вигадуйте сцену');
...
describe('bannedPhrasesFor', () => {
  ...
  it('every EN rule fires on realistic template-leak text', () => {
    ...
    for (const rule of BANNED_PHRASES_EN) {
      ...
      expect(rule.pattern.test(sample), `rule ${rule.code} did not match its own sample`).toBe(
        true,
      );
    }
  });
  ...
  it('every UK rule fires on realistic template-leak text', () => {
    const samples: Record<string, string> = {
      ...
      label_opener_limitation:
        'Обмеження полягає в тому, що звіт стосується лише одного постачальника.',
      label_opener_takeaway:
        'Висновок для рішення: ізоляцію потрібно перевіряти, а не вважати гарантованою.',
  ...
  it('the UK label-opener rules match the exact phrases from the rejected 2026-07-27 edition body', () => {
    const limitationRule = BANNED_PHRASES_UK.find(
      (rule) => rule.code === 'label_opener_limitation',
    )!;
    expect(
  ...
  it('does not false-positive on ordinary prose', () => {
    ...
    for (const rule of BANNED_PHRASES_EN) {
      expect(
        rule.pattern.test(cleanEn),
        `rule ${rule.code} false-positived on clean EN prose`,
      ).toBe(false);
    }
    for (const rule of BANNED_PHRASES_UK) {
      expect(
        rule.pattern.test(cleanUk),
        `rule ${rule.code} false-positived on clean UK prose`,
      ).toBe(false);
    }
  });
});
