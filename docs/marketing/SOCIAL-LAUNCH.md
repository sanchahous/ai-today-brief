...
**Дата:** 2026-06-11 · **Базується на:** research офіційних доків кожної платформи (джерела внизу кожної секції зібрані агентами; цифри/ліміти станом на червень 2026 — критичні значення звіряй з офіційною сторінкою в момент виконання, платформи змінюють правила часто).
...
**Наскрізне правило неймінгу:** усюди один хендл **@aitodaybrief** (фолбеки: `@aitodaybrief_`, `@ai_today_brief`). ⚠️ Instagram-хендл назавжди = Threads-хендл, обирай як фінальне бренд-рішення.
...
- [ ] Експортнути PNG з бренд-кіта (`docs/marketing/brand-kit/` → інструкція в README там же): аватар 1024², банери X/LinkedIn/YouTube/Bluesky/Facebook.
...
**Кроки:**
1. IG-акаунт на бренд-пошту, хендл `@aitodaybrief` (⚠️ він же назавжди = Threads-хендл).
...
**Зараз (30 хв):** Google-акаунт бізнесовий + 2FA → Create a channel → **«Use a custom name»** = Brand Account «AI Today Brief» (незалежний від особистого Google, можна додавати менеджерів, переживе передачу) → хендл `@aitodaybrief` на youtube.com/handle → верифікація телефоном (youtube.com/verify) → оформлення: аватар 800×800, банер 2560×1440 (⚠️ **тільки центральні 1546×423 видно на всіх девайсах** — wordmark строго туди; бренд-кіт врахував), опис (нижче), лінки (перший = кнопка на хедері: сайт, далі Beehiiv/Telegram), keywords, made-for-kids = No.
...
Файли: `docs/marketing/brand-kit/` (SVG, експорт у PNG — інструкція в README.md поруч):
