...
> **Статус виконання — W0 закрито 2026-08-13** (гілка `chore/visual-v11-hygiene`).
> Знахідки нижче описують стан, у якому PR #229 був **на момент review**, і навмисно не
> переписані — це аудит, а не changelog. Що змінилось після W0:
> видалено 83 МБ бінарників, 35 one-shot скриптів, 3 осиротілі модулі і 66 з 67 workflow
> (лишився один `workflow_dispatch`-харнес); стале `artifacts/visual-affordance-v10-owner-review-complete/`
...
> за однаковими правилами. W1–W5 попереду.
>
> ⚠️ **Коригує попередню редакцію цієї сторінки (2026-08-14):** тут і в кількох інших місцях
> було написано, що продакшн `pipeline/card-image.ts` «у порівнянні не брав участі жодного
> разу». **Це неправда.** У прогонах v6 і v7 гілка `current` — це і є продакшн:
> `scripts/visual-compiler-v6-render-ab.ts:310` викликає `generateWeeklyReportageIllustrations`
> з `pipeline/card-image.ts`, і власний render-report підтверджує це таймінгом
> (1326,6 с на 7 викликів ≈ 190 с/виклик — характерна латентність FLUX, а не SVG).
> Твердження справедливе **лише** для пізніх targeted-прогонів V10, де порівнювали v10 проти v8
> і продакшн-гілку прибрали. Наслідок для W4: третю гілку не треба «додавати з нуля» — треба
> повернути ту, що вже була в v6/v7 харнесі.
...
- **Виміряно у W0 (2026-08-13):** 17 test-файлів, 121 тест, по 14 із 15 інструментованих модулів —
  **83.17% statements · 72.03% branches · 94.09% functions · 86.62% lines**. Тобто новий код
  формально перевищує 70%-гейт, і це варто знати: проблема цього PR не в кількості покриття, а в
  тому, **що саме** покрито — див. §8, де перелічені шляхи без жодного тесту (null-гілка
  `selectAffordanceTreatmentV10`, 12 із 16 issue-кодів валідатора, рендер на нестандартних
  розмірах) і тести, що закріплюють баги.
  Команда відтворення:
  `npx vitest run --coverage --coverage.all --coverage.include='**/weekly-digest/visual-*.ts' --coverage.exclude='**/*.test.ts' src/lib/weekly-digest/visual-*.test.ts`
- Постійно додавати visual-v11 у `LOGIC_INCLUDE` — тільки коли модуль стане production-path (W5), тоді ціль ~80% на новий код.
- **Готово:** ✅ число зафіксоване тут і в PR body замість «coverage passed».
