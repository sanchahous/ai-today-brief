...
| Картинки карток / weekly story | Cloudflare Workers AI **FLUX.2 klein** (`@cf/black-forest-labs/flux-2-klein-9b`); без ключа — брендований duotone-fallback. Weekly story-ілюстрації — policy **`weekly-editorial-concept-v2`** (2026-08-11): essence → metaphor (`motif_class` / `subject_kind`) → sibling structural gates → subject-first BFL SASC; `dual_contrast` max 1/digest; 3 варіанти з per-variant vision pick. Сцен-бриф через `generateWithRegistry` (LLM provider registry Phase 2). **Content Sim:** vision critic loop ≤5 (`CONTENT_SIM_MAX_IMAGE_REPAIR`) → human review; див. [content-sim](pipeline/content-sim.md) | `.env.example`, PR #169–#171, editorial quality overhaul PR5, [card-images](marketing/card-images.md), [llm-providers](pipeline/llm-providers.md), [content-sim](pipeline/content-sim.md) |
