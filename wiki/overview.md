...
| Картинки карток / weekly story | Cloudflare Workers AI **FLUX.2 klein** (`@cf/black-forest-labs/flux-2-klein-9b`); без ключа — брендований duotone-fallback. Weekly story-ілюстрації — policy **`weekly-semantic-story-v5.1`** (2026-08-12): approved story/research → semantic contract **context → meaning → mechanism → consequence** → three-seat concept jury (`literal_context` / `mechanism` / `consequence`) → окремий subject-first FLUX prompt на кожен сценарій. Structural duplication/opaque motifs блокуються до render; craft-cliché (`terminal`, collage, glowing brain) блокує лише якщо це **не** буквальний предмет новини (B1-fix 2026-08-15); журі не добиває кількість трьома фолбеками з одного essence, а дублікати ловляться за родиною мотивів (B2 2026-08-15); semantic token mismatch є advisory для vision. Якщо critic відхиляє метафору, його scene/patches стають feedback нового jury і не копіюються в усі три prompts. Vision окремо оцінює кожен concept, вимагає pixel evidence, а overall затискається найслабшим semantic score. `dual_contrast` max 1/digest; максимум 2 раунди по 3 паралельні concept renders і 3 parallel vision reviews; суцільний semantic fail планує нову трійку. Provider-call ledger показує current-run і cumulative story-revision spend. Сцен-бриф через `generateWithRegistry` (LLM provider registry Phase 2). Див. [content-sim](pipeline/content-sim.md) | `.env.example`, `pipeline/card-image.ts`, `generation-worker.ts`, [card-images](marketing/card-images.md), [llm-providers](pipeline/llm-providers.md), [content-sim](pipeline/content-sim.md) |
