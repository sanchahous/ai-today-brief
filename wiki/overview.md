...
| Картинки карток / weekly story | Cloudflare Workers AI **FLUX.2 klein** (`@cf/black-forest-labs/flux-2-klein-9b`); без ключа — брендований duotone-fallback. Weekly story-ілюстрації — policy **`weekly-semantic-story-v4`** (2026-08-11): approved story/research → semantic contract **context → meaning → mechanism → consequence** → causal physical metaphor → subject-first FLUX prompt. Gate перевіряє лише поля, які реально доходять до render; vision окремо оцінює `context_fidelity`, `mechanism_legibility`, `consequence_legibility`, `instant_comprehension`, а overall затискається найслабшим semantic score. `dual_contrast` max 1/digest; 3 seed-варіанти з per-variant vision pick; repair patch застосовується до наступного реального FLUX request. Сцен-бриф через `generateWithRegistry` (LLM provider registry Phase 2). **Content Sim:** vision critic loop ≤5 (`CONTENT_SIM_MAX_IMAGE_REPAIR`) → human review; див. [content-sim](pipeline/content-sim.md) | `.env.example`, `pipeline/card-image.ts`, `generation-worker.ts`, [card-images](marketing/card-images.md), [llm-providers](pipeline/llm-providers.md), [content-sim](pipeline/content-sim.md) |
