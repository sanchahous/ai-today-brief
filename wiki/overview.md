...
| Картинки карток / weekly story | Cloudflare Workers AI **FLUX.2 klein** (`@cf/black-forest-labs/flux-2-klein-9b`); без ключа — брендований duotone-fallback. Weekly story-ілюстрації від PR5 (2026-08-06) — окрема репортажна house-style (policy `weekly-reportage-v1`, 3 варіанти на історію). Сцен-бриф (текстовий крок перед зображенням) від LLM provider registry Phase 2 (2026-08-06) іде через `generateWithRegistry` — той самий уніфікований реєстр провайдерів, що й для решти LLM-викликів проєкту | `.env.example`, PR #169–#171, editorial quality overhaul PR5, [card-images](marketing/card-images.md), [llm-providers](pipeline/llm-providers.md) |
