...
| Картинки карток / weekly story | **Новини на сайті** лишаються на Cloudflare Workers AI **FLUX.2 klein** (`@cf/black-forest-labs/flux-2-klein-9b`); без ключа — брендований duotone-fallback. Origin карток з 2026-08-15 — JPEG 1280×720 q82 (`encodeCardOrigin`), не ~488 КБ PNG; історичні `.png` переписує `--reencode-png` без FLUX. Сайт з 2026-08-17 віддає WebP через `image-loader` (`format=webp`); weekly `story_image` origin — WebP 1600×900, cover лишається JPEG бо це `og:image`. Модель новин не змінювали. **Daily cover** (2026-08-15, P3): один LLM-виклик `daily.cover_scene` на випуск → `briefs.cover_prompt` + Telegram `<pre>`, **без** авто-рендеру обкладинки. **Weekly story/cover** за замовчуванням `WEEKLY_STORY_IMAGE_MODE=prompt_only` (2026-08-15): джоба пише `story_prompt_set` і чекає upload, без FLUX; `render` повертає старий цикл. Планування концептів — policy **`weekly-semantic-story-v6`** (2026-08-23, Prompt-as-Code): approved story/research → semantic contract **context → meaning → mechanism → consequence** → three-seat concept jury (`literal_context` / `mechanism` / `consequence`) → copy-ready prompts (або FLUX, якщо `render`). Structural duplication/opaque motifs блокуються до render; craft-cliché блокує лише якщо це **не** буквальний предмет новини — і лише для `terminal` (єдиний термін зі списку з нескрінним значенням); решта списку (dashboard, IDE, collage, glowing brain…) лишається безумовною забороною (B1-fix 2026-08-15, звужено review-фіксом 2026-08-15); журі не добиває кількість трьома фолбеками з одного essence, а дублікати ловляться за родиною мотивів через реальні `subject`/`setting`, не `sceneSummary`-фолбек (B2 2026-08-15, крос-story matching полагоджено review-фіксом 2026-08-15); C1 (2026-08-15) ставить `grammar` на бриф (`cinematic_domain_scene` / `source_led_fallback`), кап на лінзу `mechanism` — не всі три концепти (review-фікс 2026-08-15); C2 (2026-08-15) обирає схему, коли в title/summary або essence є точна метрика, ігноруючи duration у practical; C3 (2026-08-15) не пускає в `story_prompt_set` концепт без таблиці source → visible object → outcome (`visibleElementId`); D2 (2026-08-15) після upload радить inpaint / той самий промпт / інший концепт, без auto-repair; D3 (2026-08-15) `human_dignity_risk` валить новини і попереджає на upload; E1 (2026-08-15) вердикт власника на концепт (`used` / `used_with_edits` / `rejected` + `reasonTags`) пишеться в `story_prompt_set` і в metadata upload; E2 (2026-08-15) auto-critic спершу image-only, потім story-aware; E3 (2026-08-15) Visuals рахує якість промптів (≥60% прийнятних з 1–2 спроби, 0 misleading, ≤10 хв/story, 3 різні) і **не** блокує реліз; semantic token mismatch є advisory для vision. Якщо critic відхиляє метафору, його scene/patches стають feedback нового jury і не копіюються в усі три prompts. У режимі `render` vision окремо оцінює кожен concept, вимагає pixel evidence, а overall затискається найслабшим semantic score. `dual_contrast` max 1/digest; максимум 2 раунди по 3 паралельні concept renders і 3 parallel vision reviews; суцільний semantic fail планує нову трійку. Provider-call ledger показує current-run і cumulative story-revision spend. Сцен-бриф через `generateWithRegistry` (LLM provider registry Phase 2). Див. [content-sim](pipeline/content-sim.md) | `.env.example`, `pipeline/card-image.ts`, `pipeline/daily-cover-prompt.ts`, `generation-worker.ts`, [card-images](marketing/card-images.md), [llm-providers](pipeline/llm-providers.md), [content-sim](pipeline/content-sim.md) |
| X (Twitter) постинг | hard cap **≤ €10/міс** у БД; резервація `X_POST_ESTIMATED_COST_EUR=0.40` на пост | `.env.example` |
| Weekly master LLM | оцінка $3/M input, $15/M output; kill-switch `WEEKLY_CONTENT_STUDIO_V2=off`; hard cap `WEEKLY_MASTER_MAX_SPEND_USD` (default $4); per-M стеля `OPENROUTER_MAX_PRICE_PER_MILLION=1.5` | `.env.example`, PR #163, `generation-worker.ts` |
| Social writer/critic LLM | стеля `SOCIAL_LLM_MAX_PRICE_PER_MILLION=1.5` blended; каталожний ранкер (якість під стелею); `:free` у черзі; `OPENROUTER_CACHE_HIT_RATE=0.182` | `.env.example`, [research/2026-08-30-openrouter-routing-api](research/2026-08-30-openrouter-routing-api.md) §12 |
...
створюють один primary cause-and-effect кандидат; rejection запускає конкретний repair, а не
вибір із трьох. Manual `story_image` після clean pixel QA має advisory story-aware semantic QA;
без завершеного semantic pass або за будь-якого active QA blocker файл не auto-attest-иться.
Картинка має разом із headline пояснювати `actor/system → visible change → grounded consequence`,
а не містити весь текст новини. (source: `src/lib/weekly-digest/story-prompt-job.ts`; `src/lib/weekly-digest/generation-worker.ts`; `src/lib/weekly-digest/run-post-upload-qa.ts`; `src/lib/weekly-digest/machine-attest.ts`; [gpt-image-prompt-plan-review](audits/2026-08-23-gpt-image-prompt-plan-review.md))
...
- [card-images](marketing/card-images.md) — картки / weekly story images
- [image-prompt-library](pipeline/image-prompt-library.md) — Prompt-as-Code v6
