...
набір символів; зʼясувалось, що це навмисний click-tracking редирект (окрема сутність),
а справжня канонічна сторінка вже мала людський slug — але лише дата
(`ai-weekly-2026-08-16`), без жодного слова про тему випуску. Причина: slug
присвоюється в `composer.ts` у момент створення дайджесту, задовго до появи заголовка.

Рішення власника: правити тільки для нових випусків наперед (старі URL не чіпати — вже
в Google), slug генерувати автоматично з заголовка. Реалізовано в
`finish_weekly_digest_release` (єдина функція, що ставить `status='published'`
незалежно від шляху — ручний Ship чи `release_at`-воркер): якщо поточний slug ще
збігається з авто-паттерном `ai-weekly(-test)?-YYYY-MM-DD`, переписує на
`{транслітерований-заголовок}-YYYY-MM-DD`. Деталі —
...
детермінований гейт уже в repair-промпті — і впав `telegram_radar_block_required` (якір
`^Радар` на першому рядку). Тіло відхиленої копії на джобі не зберігається; той самий якір не
спіймав би й «На радарі». Критик окремо зрізав `platform_fit` до 75 за **нуль** замість цифри
(це лишається судженням критика, не кодом).

Коригування: перший рядок блоку **містить** Топ 3 / Радар (локатив «радарі» теж), не мусить
ними починатись; слово «Радар» більше не є blocking, якщо Топ 3 вже окремий блок — інакше
гейт валить валідну верстку, якої просив контракт прозою, а не цей інцидент. Код
`telegram_radar_block_required` прибрано з always-block. CTA ≤180 і ≥4 блоки без змін.
(source: прод-Supabase job `3f8a1db2-883e-41c7-84de-d834ac50ac24` 2026-08-28 14:35 UTC;
GitHub Actions `33180626567`)
