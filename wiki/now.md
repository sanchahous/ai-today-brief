...
1. PR1–5 закомічені й запушені на PR [#189](https://github.com/sanchahous/ai-today-brief/pull/189)
   (одна гілка `feat/weekly-editorial-voice`, комітяться туди послідовно). PR5 = репортажні
   ілюстрації + вибір з 3 варіантів (`pipeline/card-image.ts`, повністю окремо від daily-шляху).
2. **Дві зупинки перед мержем/рол-аутом, обидві чекають рішення власника (реальні платні
   виклики, не запущені самостійно):**
   - PR3: critic-only shadow-прогін на збереженому master `ai-weekly-2026-07-27` — новий критик
     має провалити його по voice/engagement, інакше якорі не дискримінують.
   - PR5: dry-run 9 klein-рендерів (3 історії × 3 сіди) — власник оцінює, чи FLUX.2 klein тримає
     репортажний стиль, чи потрібен fallback (flux-2-dev або заземлена метафора).
