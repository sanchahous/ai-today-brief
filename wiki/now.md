...
1. PR1–4 закомічені й запушені на PR [#189](https://github.com/sanchahous/ai-today-brief/pull/189)
   (одна гілка `feat/weekly-editorial-voice`, комітяться туди послідовно). PR4 = кут подачі
   (`weekly_digest_story_directions`, Research tab textarea) — **міграція написана, НЕ застосована
   до живої БД** (DDL через звичайний деплой-пайплайн, не ad-hoc виклик із цієї сесії); код
   деградує без помилок, якщо таблиці ще нема.
...
3. PR5–7 (ілюстрації-репортажі, відеосценарій, соц-голос) — код продовжується, кожен зі своєю
   точкою зупинки перед live-перевіркою (PR5 — dry-run картинок, потребує рішення власника).
