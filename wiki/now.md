...
- **Supabase egress (2026-09-02).** Спайк — три `next build` проти PostgREST, не
  читачі. Орг на **Pro** з ~12:11 UTC; Data API **200**. #348 (e2e cap) + #350
  (SSG disk-memo). Прод SSG `dd49672` 13:08 UTC: Vercel Ashburn `categories`
  **2063→30**, `brief_items` **3247→1471**. Weekly `ai-weekly-2026-08-23` далі
  чекає release-due + promote, не SQL-finish.
...
- **OpenRouter spend leak виправлено (2026-08-29), гілка `claude/openrouter-activity-investigation-1d5a53`.**
  ...
  шлях, reported-вартість у ledger, глобальний `DAILY_GENERATION_BUDGET_USD=1` (ковзні 24 год)
  ...
  **`DAILY_GENERATION_BUDGET_USD=1`** (рішення 30.08) — нижче історичних днів випуску (11.08 $2.68,
  22.08 $1.53 metered), тож дайджест-день очікувано впреться в межу і її треба буде підняти вручну
  на той день; помилка `budget_exceeded` non-retryable, нічого не тарифікується. Per-role стеля
  свідомо відкладена — власник планує переписати принцип вибору моделі цілком.
...
- **Weekly digest Етап 0 ревізій (2026-08-29), гілка `feat/weekly-revision-stage-0`.**
  ...
  `#441`. `npm run migrations:check` ловить дрейф origin/main vs prod. Міграція
  `20260829130000` (не `20260829120000` — той слот зайняв #341 topic slug).
  Етап 1 — окремий проєкт.
...
- **Публічний slug weekly тепер тематичний (2026-08-29), змержено як [#341](https://github.com/sanchahous/ai-today-brief/pull/341).**
  `finish_weekly_digest_release` переписує placeholder `ai-weekly(-test)?-YYYY-MM-DD`
  на `{title}-YYYY-MM-DD` лише для нових публікацій. Старі URL не чіпаються.
  (source: [weekly-digest](pipeline/weekly-digest.md))
