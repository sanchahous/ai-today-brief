...
## 4. Lint і sync з проєктом

`npm run wiki:lint` — формат сторінки, биті посилання, сироти, сторінки поза `index.md`,
факти без джерела. Скрипт **лише звітує** (exit 0); `--strict` — ненульовий код.

`npm run wiki:sync` — **жорсткий гейт**: падає, якщо код під watcher у
`wiki/_meta/project-sync.json` новіший за пов’язані wiki-сторінки, або ключові факти з
коду/`.env.example` (модель FLUX, `WEEKLY_CONTENT_STUDIO_V2`, spend-cap, prompt policy)
немає в wiki, або лічильник міграцій / статуси ✅📋 у `index.md` розʼїхались із диском.

`npm run wiki:check` = `wiki:sync` + unit-тести sync + `wiki:lint --strict`. Входить у
`npm run pr:check`.

Семантична частина (суперечності між сторінками, концепти без власної сторінки) — агент за
запитом «run lint». **Автовиправлення заборонене.**

Контракт sync розширюють у `wiki/_meta/project-sync.json` (нові watchers / facts), коли
з’являється нова «зона правди» в коді.
