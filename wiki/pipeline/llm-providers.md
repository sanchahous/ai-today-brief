...
- **Умову плану «6b лише після ≥1 доби стабільної роботи 6a» не витримано** — 6a змержено в
  ...
  прод-циклу 6a у ролі `daily.verify` ще не спостережено.
- **Живий `auto-publish --dry-run` виконано (2026-08-07).** `--dry-run` не пише в БД
  (`applyApprove`/`applyReject`/`publishDraft` пропускаються) і не шле Telegram, але суддю
  викликає по-справжньому — найдешевша реальна перевірка перед першим нічним прогоном. Перший
  прогін з дефолтним `--window-days 7` дав `window_drafts: 0` — жодної чернетки в
  7-денному вікні прямо зараз (найновіша неопублікована — старша), тож суддя не спрацював.
  Прод має 9 старих `draft`-брифів (2026-06-10…2026-07-20) з pending-айтемами, навмисно поза
  вікном (design intent: застарілі новини не публікуються). Другий прогін,
  `--dry-run --window-days 90`, підняв їх у вікно й реально викликав суддю: **3 успішні живі
  виклики через новий шлях** `daily.auto_publish_judge` → `generateWithRegistry` →
  `loadProviderRegistry`'s `defaultChain` → OpenRouter http-провайдер (`deepseek/deepseek-v4-pro`,
  підтверджено логом `openrouter-models.ts`'s «Model queue resolved», унікальним для
  реєстрового шляху) — валідні JSON-відповіді, `finish_reason: "stop"`. БД не змінено (перевірено
  повторним читанням `brief_items`/`briefs` після прогону). Тимчасові скрипти:
  `tmp/auto-publish-6b-dryrun/check-drafts.ts`, `check-stale.ts` (author-only, не закомічені —
  той самий патерн, що `tmp/nim-http-provider-dryrun/`).
- **Побічна знахідка (не Фаза 6b, поза обсягом цієї міграції):** `runAutoPublish`'s
  ранній `return` при `windowDrafts.length === 0` жорстко ставить `staleDrafts: 0` замість
  реального підрахунку — з 9 реальними застарілими чернетками в проді цей прогін показав
  `stale_drafts: 0`. Не впливає на жодне рішення (Telegram-нотифікація про застарілі чернетки й
