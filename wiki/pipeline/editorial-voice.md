...
## Гейт числової відповідності EN↔UK (`numeric_parity`, 2026-08-09)

Sandbox-прогін показав помилку, яку критик пропустив із `parity` 88/100: EN-заголовок
«Claude Code's **600x** Energy Bill» став українською «Рахунки Claude Code **на 600%**
більше». 600% ≈ 7×, а не 600× — розбіжність на два порядки. Причому standfirst того самого
випуску казав «в 600 разів» правильно, тобто стаття суперечила сама собі, і це все одно
пройшло критика.

Новий детерміністичний блокер `numeric_parity` спрацьовує, коли та сама величина в одній
локалі — кратність, а в іншій — відсоток, і при цьому друга локаль ніде не називає її
відсотком самостійно. Свідомо вузько: відсоток, який просто відрізняється між локалями, або
ідіоматичне «на 100%» лишаються критикові.

Дві технічні пастки, закладені в тест:
- `\b` і `\w` у JS лишаються ASCII навіть із прапорцем `/u`, тож `раз(?:ів)?\b` **ніколи** не
  збігається — весь напрямок UK→EN тихо пропускав би все. Використано явний lookahead
  «не кирилична літера»;
- український десятковий кома-роздільник (`1,5`) і нерозривні пробіли в розрядах
  нормалізуються перед порівнянням, інакше кожне локалізоване число виглядає зміненим.

У промпт UK-адаптації додано відповідний рядок контракту («кратність лишається кратністю»),
але саме гейт — довговічна частина: промпт може зісковзнути, гейт ні.
(source: `src/lib/weekly-digest/content-studio.ts`, sandbox-прогін 2026-08-09,
`artifacts/_local/weekly-sandbox/2026-08-09T15-34-07-721Z/ukrainian.json`)
