...
| ✅ [pipeline/weekly-digest](pipeline/weekly-digest.md) | Weekly Content Studio v2, durable workers, linked-retry social checkpoints, score-aligned critic warnings, versioned legacy-post repair, bounded 7-page LinkedIn document, semantic story images v5.1, spend ledger і admin UX | `.env.example` + PR #160–#177/#208/#209/#222 + `social-adapter.ts` + `generation-worker.ts` + `linkedin-document.ts` |
