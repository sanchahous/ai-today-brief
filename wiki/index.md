...
| ✅ [pipeline/weekly-master-engine](pipeline/weekly-master-engine.md) | Ітеративний рушій `editorial_master`: посегментний запис, точковий ремонт поля, якість більше не валить джобу; від 2026-08-22 non-converged вихід одразу стає робочою копією; pre-critic більше не LLM-переписує вже сплайсені мовні фікси; critic крутить модель між раундами й ревізіями | `master-engine.ts` / `master-persist.ts` / `master-repair.ts` / `editorial-llm.ts`, owner session 2026-08-09 / 2026-08-22 |
