...
| ✅ [pipeline/weekly-digest](pipeline/weekly-digest.md) | Weekly Content Studio v2, durable workers, semantic story images v5: three-lens concept jury, 2×3 parallel loop, spend ledger і admin UX | `.env.example` + PR #160–#177/#208/#209/#222 + `card-image.ts` + `generation-worker.ts` |
...
| ✅ [pipeline/content-sim](pipeline/content-sim.md) | Симуляція/бектест: per-concept 2×3 parallel vision loop, pixel-evidence semantic gates, escalation, release gate | `pipeline/card-image.ts`, `src/lib/content-sim`, 2026-08-11 |
...
| ✅ [marketing/card-images](marketing/card-images.md) | Daily cards + weekly semantic illustration v5: three-lens concept jury, causal fallback, per-concept vision gate | `pipeline/card-image.ts`, owner review + smoke renders 2026-08-11 |
