...
| Механізм | Де | Що дає |
|---|---|---|
| Definition block у перших ~300 словах | концепти, гайди | Модель витягує готове визначення |
| `llms.txt` | `/llms.txt` | Карта сайту для LLM-краулерів: ключові сторінки обох мов, фіди, правила цитування |
| JSON-LD на кожному типі сторінки | NewsArticle / TechArticle / WebApplication / FAQPage / BreadcrumbList / Organization+WebSite | Машинна верифікація сутностей; FAQPage дає Q/A-пару «як є» |
...
| Чесний lastmod/dateModified | sitemap + Article schema (B10) | Свіжість — сигнал ранжування відповідей; брехливий lastmod карається недовірою |
| RSS обома мовами | `/rss.xml`, `/rss-uk.xml` (B6) | Фіди — прямий канал споживання для агентів і рідерів |
| Автолінки між концептами | `linkConceptMentions` (B9) | Граф внутрішніх лінків = граф сутностей для extraction |
| robots.txt пускає AI-ботів | GPTBot/PerplexityBot/ClaudeBot/Google-Extended | Без цього все вище не має сенсу |
...
- Реферали від AI-систем у GA4 (`session source` ∈ perplexity.ai, chatgpt.com, …) —
  базова лінійка до появи: direct/none. Події див.
