...
description: >
  Audits and optimizes application performance: bundle size, React rendering,
  data fetching waterfalls, database queries, assets, and Core Web Vitals.
  Use when the user asks to optimize performance, reduce load times, fix slow
  pages, audit Lighthouse/CWV, or profile study-platform / Vite / Supabase apps.
---
...
## Project context (portfolio monorepo)

- **study-platform**: Vite + React — use `npx vite-bundle-visualizer` from `apps/study-platform`.
- **Supabase**: check over-fetching (`content_v2` + legacy `days`), parallel fetches, N+1 in hooks.
- Run tests after changes: `npm run test:study-platform`.
...
Upstream: [awesome-cursor-skills/auditing-performance](https://github.com/spencerpauly/awesome-cursor-skills/blob/main/resources/auditing-performance/SKILL.md)
