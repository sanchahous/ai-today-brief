-- X self-reply is 280 characters. Rewriting copy onto the full UTM URL
-- leaves no room for the USE line, so the reply collapses to a bare link.
-- Keep Destination/utm_url fully tagged; rewrite X copy onto page + s=.
...
begin
  ...
  loop
    ...
    v_copy_tracked := case
      when v_post.channel = 'x' then public.with_social_click_token(v_page, v_post.tracking_token)
      else v_tracked
    end;
