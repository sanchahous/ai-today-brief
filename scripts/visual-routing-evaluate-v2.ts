...
function extractJson(text: string) {
  ...
  try {
    ...
    throw new Error(`Invalid JSON: ${clean.slice(0, 400)}`);
  }
}
    ...
    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'X-Title': 'AI Today Brief bounded visual backtest',
        },
        body: JSON.stringify({
          model: MODEL,
      ...
      lastError = error;
      console.warn(`[judge-v2] attempt ${attempt} failed`, error);
    ...
    [
      {
        type: 'text',
        text:
          'Inspect pixels only. No headline or prompt is available. Fill the exact schema. Keep arrays short. For repeated or gibberish text, do NOT transcribe every occurrence: set text_quality and give one short unique clue only. State only visible evidence; do not infer product names.',
      },
      {
        type: 'image_url',
    ...
    [
      {
        type: 'text',
        text:
          'Use only the blind observation. Select one candidate. Context, mechanism and consequence pass only when visibly supported, not merely compatible. instant_pass requires a coherent context→mechanism→consequence mini-story understandable in about three seconds. OBSERVATION=' +
...
async function evaluate(row: ManifestRow): Promise<Evaluation> {
  ...
  return { storyId: row.storyId, variant: row.variant, imagePath: path, observation, verdict, retrievalCorrect, allSemanticPass };
}
...
function metrics(evaluations: Evaluation[]) {
  return (['baseline', 'routed', 'hybrid'] as Variant[]).map((variant) => {
    ...
    return {
      ...
      clean_text_rate: rate(rows, (row) => !row.observation.text_present || row.observation.text_quality === 'clean'),
    };
  });
}
...
async function main() {
  ...
  const aggregate = metrics(evaluations);
  await writeFile(join(ROOT, 'evaluations-v2.json'), JSON.stringify(evaluations, null, 2));
  await writeFile(join(ROOT, 'metrics-v2.json'), JSON.stringify(aggregate, null, 2));
  const report = [
    '# Visual routing backtest — bounded blind judge',
    '',
    '| Variant | Retrieval | Context | Mechanism | Consequence | 3-second story | All gates | Clean/no text |',
    '|---|---:|---:|---:|---:|---:|---:|---:|',
    ...aggregate.map(
      (row) =>
        `| ${row.variant} | ${pct(row.retrieval_accuracy)} | ${pct(row.context_pass_rate)} | ${pct(row.mechanism_pass_rate)} | ${pct(row.consequence_pass_rate)} | ${pct(row.instant_pass_rate)} | ${pct(row.all_semantic_gates_rate)} | ${pct(row.clean_text_rate)} |`,
    ),
    '',
    '## Per image',
    '',
    '| Story | Variant | Selected | Context | Mechanism | Consequence | 3s | Text quality |',
    '|---|---|---|---:|---:|---:|---:|---|',
    ...evaluations.map(
      (row) =>
  ...
  await writeFile(join(ROOT, 'RESULTS-V2.md'), `${report}\n`);
  console.log(report);
}
