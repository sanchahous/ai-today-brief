...
async function renderCurrent(record: V5ClaimRecord): Promise<{
  bytes: Buffer;
...
async function renderCompiler(record: V5ClaimRecord): Promise<{
  pixels: Buffer;
  final: Buffer;
  audit: VariantAudit;
}> {
  ...
  const mode = record.renderMode || selectVisualRenderModeV5(record.autoClaim);
...
function report(rows: RenderRow[]): string {
  ...
  const lines = [
    '# Visual Compiler v5 fresh A/B render',
    '',
    `Stories: **${rows.length}**.`,
...
async function main() {
  ...
  const claims = JSON.parse(await readFile(INPUT_PATH, 'utf8')) as V5ClaimRecord[];
  ...
  for (const [index, record] of [...claims].sort((a, b) => a.story.rank - b.story.rank).entries()) {
    ...
    console.log(`[v5-ab] ${index + 1}/${claims.length} current: ${record.story.title}`);
    const current = await renderCurrent(record);
    console.log(`[v5-ab] ${index + 1}/${claims.length} compiler: ${record.renderMode}`);
    ...
    const compilerIsX = seedFromString(`v5-blind:${record.story.revision_item_id}`) % 2 === 0;
