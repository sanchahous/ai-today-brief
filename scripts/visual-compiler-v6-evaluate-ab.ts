...
const MODEL =
  process.env.VISUAL_V6_JUDGE_MODEL?.trim() || 'google/gemini-2.5-flash';
    ...
    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'X-Title': 'AI Today Brief v6 targeted visual evaluation',
        },
        body: JSON.stringify({
          model: MODEL,
      ...
      lastError = error;
      console.warn(`[v6-eval] attempt ${attempt} failed`, error);
...
async function evaluatePixels(
  ...
  const compilerIsX = seed(`v6-targeted-pixel:${manifest.storyId}`) % 2 === 0;
...
function sourceEvaluation(
  ...
  return {
    ...
    visualPass: visual,
    productionPass: visual && (source === 'current' || claimEligible),
...
function report(rows: EvaluationRow[], usage: UsageTotals): string {
  ...
  const metric = (source: Source, key: 'pixelPass' | 'visualPass' | 'productionPass') =>
  ...
  const lines = [
    ...
    `Eligible compiler claims before rendering: **${rows.filter((row) => row.claimEligible).length}/${count}**.`,
    '',
  ...
  for (const row of rows) {
    lines.push(
      `| ${row.rank} | ${row.headline.replace(/\|/g, '\\|')} | \`${row.role}\` | \`${row.mappingMode}\` | ${row.claimEligible ? '✓' : '✕'} | ${mark(row.current.pixelPass)}/${mark(row.current.visualPass)}/${mark(row.current.productionPass)}/${row.current.weightedScore.toFixed(1)} | ${mark(row.compiler.pixelPass)}/${mark(row.compiler.visualPass)}/${mark(row.compiler.productionPass)}/${row.compiler.weightedScore.toFixed(1)} | **${row.preferredSource}** | ${row.reason.replace(/\|/g, '\\|')} |`,
    );
  }
...
async function evaluatedSheet(rows: EvaluationRow[], manifest: ManifestRow[]): Promise<Buffer> {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    labels.push(
      ...
      `<text x="${margin}" y="${y + headerH + cardH + 58}" font-family="DejaVu Sans,Arial,sans-serif" font-size="13" fill="${row.claimEligible ? '#6EE7B7' : '#FDA4AF'}">claim ${row.claimEligible ? 'PASS' : 'FAIL'} • ${xml(row.role)} • ${xml(row.mappingMode)}</text>`,
    );
  }
  labels.push('</svg>');
...
function treatmentAudit(claim: ClaimRecord): ClaimAudit {
  const issues = [
    ...claim.deterministicIssues,
    ...(claim.treatment.analogyAudit?.issues ?? []),
  ];
  return {
    storyId: claim.story.revision_item_id,
    passed: claim.treatment.eligible,
    issues,
    rationale:
      claim.treatment.analogyAudit?.rationale || claim.treatment.reason,
  };
}
...
async function main() {
  ...
  const [manifest, claims] = await Promise.all([
    ...
    readFile(CLAIMS_PATH, 'utf8').then(
      (value) => JSON.parse(value) as ClaimRecord[],
    ),
  ]);
  if (manifest.length !== 5) {
    throw new Error(`Expected 5 targeted manifest rows; got ${manifest.length}.`);
  }
  const claimById = new Map(
    claims.map((claim) => [claim.story.revision_item_id, claim] as const),
  );
  const rows: EvaluationRow[] = [];
  const usage = emptyUsage();

  for (const [index, item] of [...manifest]
    .sort((left, right) => left.rank - right.rank)
    .entries()) {
    const claim = claimById.get(item.storyId);
    if (!claim) throw new Error(`Missing treatment for ${item.storyId}.`);
    console.log(
      `[v6-eval] ${index + 1}/${manifest.length} pixels: ${item.headline}`,
    );
    const pixels = await evaluatePixels(item, claim, usage);
    console.log(`[v6-eval] ${index + 1}/${manifest.length} cards`);
    const cards = await evaluateCards(item, claim, usage);
    const pixelBySource = mapBySource(pixels.order, {
      X: pixels.X,
      Y: pixels.Y,
    });
    ...
    const cardBySource = mapBySource(cardOrder, {
      X: cards.X,
      Y: cards.Y,
    });
    const eligible = claim.treatment.eligible;
    rows.push({
      ...
      claimAudit: treatmentAudit(claim),
      current: sourceEvaluation(
        'current',
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      await evaluatedSheet(rows, manifest),
    ),
  ]);
