...
const ROOT =
  process.env.VISUAL_V5_AB_OUT_DIR?.trim() || 'artifacts/visual-compiler-v5-fresh-ab';
const CLAIMS_PATH =
  process.env.VISUAL_V5_CLAIMS?.trim() ||
    ...
    try {
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'X-Title': 'AI Today Brief v5 role-aware visual evaluation',
        },
        body: JSON.stringify({
          model: MODEL,
      ...
      lastError = error;
      console.warn(`[v5-eval] attempt ${attempt} failed`, error);
...
async function main() {
  ...
  for (const [index, item] of [...manifest].sort((a, b) => a.rank - b.rank).entries()) {
    ...
    console.log(`[v5-eval] ${index + 1}/${manifest.length} pixels: ${item.headline}`);
    const pixels = await evaluatePixels(item, claim, usage);
    console.log(`[v5-eval] ${index + 1}/${manifest.length} cards`);
    ...
    rows.push({
      ...
      current: sourceEvaluation('current', pixelBySource.current, cardBySource.current, claim.eligible),
